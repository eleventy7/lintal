//! HiddenField checkstyle compatibility tests.
...
use lintal_linter::rules::HiddenField;
use lintal_linter::{CheckContext, Rule};
use lintal_source_file::{LineIndex, SourceCode};
use regex::Regex;
use test_harness::TestResult;

/// Run the HiddenField rule on source code and return violation lines.
fn check_hidden_field(source: &str) -> Vec<usize> {
    let mut parser = JavaParser::new();
    let Some(result) = parser.parse(source) else {
        panic!("Failed to parse source");
    };

    let rule = HiddenField::default();
...
fn load_fixture(file_name: &str) -> Option<String> {
    let path = checkstyle_repo::coding_test_input("hiddenfield", file_name)?;
    std::fs::read_to_string(&path).ok()
}

#[test]
fn test_input_hidden_field2() {
    let Some(source) = load_fixture("InputHiddenField2.java") else {
        eprintln!("Skipping test: checkstyle repo not available");
        return;
    };

    let expected = parse_expected_violations(&source);
    let actual = check_hidden_field(&source);

    let result = TestResult::compare(expected, actual);
    result.print_report("InputHiddenField2.java");
...
#[test]
fn test_method_parameter_hides_field() {
    let source = r#"
class Test {
    private int value;
...
"#;
    let violations = check_hidden_field(source);
    assert_eq!(
        violations.len(),
        1,
        "Method parameter hiding field should be a violation"
    );
}

#[test]
fn test_local_variable_hides_field() {
    let source = r#"
class Test {
    private int count;
    public void method() {
...
"#;
    ...
    assert_eq!(
        violations.len(),
        1,
        "Local variable hiding field should be a violation"
    );
}

#[test]
fn test_no_hidden_field_no_violation() {
    let source = r#"
class Test {
    private int value;
    public void method(int other) {
        int local = 0;
    }
}
"#;
    ...
    assert!(
        violations.is_empty(),
        "No hidden fields should have no violations"
    );
}
