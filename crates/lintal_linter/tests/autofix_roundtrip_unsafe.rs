//! Autofix roundtrip tests for UNSAFE fixes.
//!
//! These tests verify that lintal's unsafe auto-fix functionality produces
//! correct, compilable Java code. For each fixture:
//! 1. Compile original Java file (must succeed)
//! 2. Run lintal fix --unsafe
...
//!
//! Unsafe fixes are kept separate from safe fixes to make it easy to
//! validate and review which fixes require the --unsafe flag.
...
/// Run the roundtrip test for a single fixture with --unsafe flag.
fn test_fixture_unsafe(fixture: &Fixture) -> Result<(), String> {
    ...
    // Step 2: Run lintal fix --unsafe
    let output = Command::new(&lintal)
        .arg("fix")
        .arg("--unsafe") // KEY DIFFERENCE: apply unsafe fixes
        .arg(&test_file)
        .arg("--config")
        .arg(&fixture.checkstyle_xml)
        .output()
        .map_err(|e| format!("Failed to run lintal fix --unsafe: {}", e))?;

    if !output.status.success() {
        return Err(format!(
            "lintal fix --unsafe failed:\nstdout: {}\nstderr: {}",
...
#[test]
fn test_autofix_roundtrip_unsafe() {
    if !javac_available() {
        if std::env::var("CI").is_ok() {
            panic!("javac not found in CI environment - Java setup may be missing");
        }
        eprintln!("Skipping autofix roundtrip unsafe test: javac not found in PATH");
        return;
    }
    ...
    let fixtures_dir =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/autofix_unsafe");
    ...
    if !fixtures_dir.exists() {
        ...
        eprintln!(
            "Skipping autofix roundtrip unsafe test: fixtures directory not found at {:?}",
            fixtures_dir
        );
        return;
    }
    ...
    println!("Found {} unsafe fix fixtures", fixtures.len());
    ...
    for fixture in &fixtures {
        print!("Testing {} (unsafe)... ", fixture.name);
        match test_fixture_unsafe(fixture) {
            Ok(()) => println!("OK"),
            Err(e) => {
                println!("FAILED");
