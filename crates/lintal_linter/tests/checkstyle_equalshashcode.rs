//! EqualsHashCode checkstyle compatibility tests.
...
use lintal_linter::rules::EqualsHashCode;
use lintal_linter::{CheckContext, Rule};
use lintal_source_file::{LineIndex, SourceCode};
use regex::Regex;
use test_harness::TestResult;

/// Run the EqualsHashCode rule on source code and return violation lines.
fn check_equals_hashcode(source: &str) -> Vec<usize> {
...
fn load_fixture(file_name: &str) -> Option<String> {
    let path = checkstyle_repo::coding_test_input("equalshashcode", file_name)?;
    std::fs::read_to_string(&path).ok()
}

#[test]
fn test_input_equals_hashcode() {
    let Some(source) = load_fixture("InputEqualsHashCode.java") else {
        eprintln!("Skipping test: checkstyle repo not available");
        return;
    };

    let expected = parse_expected_violations(&source);
    let actual = check_equals_hashcode(&source);

    let result = TestResult::compare(expected, actual);
    result.print_report("InputEqualsHashCode.java");
...
#[test]
fn test_input_equals_hashcode_no_equals() {
    let Some(source) = load_fixture("InputEqualsHashCodeNoEquals.java") else {
        eprintln!("Skipping test: checkstyle repo not available");
        return;
    };
    ...
    let result = TestResult::compare(expected, actual);
    result.print_report("InputEqualsHashCodeNoEquals.java");
...
"#;
    let violations = check_equals_hashcode(source);
    assert_eq!(
        violations.len(),
        1,
        "Class with equals() but no hashCode() should have violation"
    );
}
...
"#;
    ...
    assert_eq!(
        violations.len(),
        1,
        "Class with hashCode() but no equals() should have violation"
    );
}

#[test]
fn test_both_equals_and_hashcode_no_violation() {
    let source = r#"
class Test {
    public boolean equals(Object o) {
        return true;
...
"#;
    ...
    assert!(
        violations.is_empty(),
        "Class with both equals() and hashCode() should have no violations"
    );
}
