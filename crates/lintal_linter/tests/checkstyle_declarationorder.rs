//! DeclarationOrder checkstyle compatibility tests.
...
use lintal_linter::rules::DeclarationOrder;
use lintal_linter::{CheckContext, Rule};
use lintal_source_file::{LineIndex, SourceCode};
use regex::Regex;
use test_harness::TestResult;

/// Run the DeclarationOrder rule on source code and return violation lines.
fn check_declaration_order(source: &str) -> Vec<usize> {
...
fn load_fixture(file_name: &str) -> Option<String> {
    let path = checkstyle_repo::coding_test_input("declarationorder", file_name)?;
    std::fs::read_to_string(&path).ok()
}

#[test]
fn test_input_declaration_order_default1() {
    let Some(source) = load_fixture("InputDeclarationOrderDefault1.java") else {
        eprintln!("Skipping test: checkstyle repo not available");
        return;
    };

    let expected = parse_expected_violations(&source);
    let actual = check_declaration_order(&source);

    let result = TestResult::compare(expected, actual);
    result.print_report("InputDeclarationOrderDefault1.java");
...
"#;
    let violations = check_declaration_order(source);
    assert!(
        violations.is_empty(),
        "Correct declaration order should have no violations"
    );
}

#[test]
fn test_instance_field_after_constructor_violation() {
    let source = r#"
class Test {
    public Test() {}
    private int instanceField;
...
"#;
    ...
    assert!(
        !violations.is_empty(),
        "Instance field after constructor should be a violation"
    );
}

#[test]
fn test_static_field_after_instance_field_violation() {
    let source = r#"
class Test {
    private int instanceField;
    public static int STATIC_FIELD = 1;
...
"#;
    ...
    assert!(
        !violations.is_empty(),
        "Static field after instance field should be a violation"
    );
}
